...
# Configurations
DATA_PATH = "/opt/airflow/data/deaths-by-accident-type.csv"
LOCAL_TRANSFORMED_PATH = "/opt/airflow/data/transformed/people_killed_by_accident_type/"
BUCKET_NAME = "indian_road_accidents-sandbox-449108"
PROJECT_ID = "kestra-sandbox-449108"
DATASET = "indian_road_accidents"
TABLE_NAME = "people_killed_by_accident_type"
...
def transform_people_killed_data(**kwargs):
    ...
    for year in missing_years:
        df_year = df[["Type of Collision", str(year)]].rename(columns={
            str(year): "people_killed",
            "Type of Collision": "type_of_collision"
        })
        
        df_year["year"] = year
        df_year = df_year[["year", "type_of_collision", "people_killed"]]
...
def upload_people_killed_data(**kwargs):
    ...
    for year in missing_years:
        ...
        blob = bucket.blob(f"people_killed_by_road_accidents/raw/{year}/{file_name}")
        blob.upload_from_filename(local_file, content_type="text/csv")

        print(f"Uploaded {file_name} to GCS under people_killed_by_road_accidents/raw/{year}/")
...
def cleanup_transformed_files():
    """Delete all transformed CSV files from the local directory."""
...
with DAG(
    dag_id="people_killed_by_accidents_dag",
    ...
    transform_task = PythonOperator(
        task_id="transform_people_killed_data",
        python_callable=transform_people_killed_data
    )

    upload_task = PythonOperator(
        task_id="upload_people_killed_data",
        python_callable=upload_people_killed_data
    )
    ...
    # DAG execution order
    check_missing_years_task >> transform_task >> upload_task >> trigger_spark_task >> check_spark_status_task
